...
export default function TeacherClasses() {
  ...
  const visibleStudents = (overview.data?.students || []).filter((item: any) => (
    graduationFilter === 'all' || (graduationFilter === 'ready' ? item.graduation_ready : item.graduation_risk)
  ))
  ...
  return <div className="space-y-6">
    ...
    <section className="railway-card grid gap-4 p-5 md:grid-cols-4">
    ...
    {overview.data && <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <AbilityRadar items={overview.data.ability_distribution} title="班级能力雷达" description="班级能力均值与各维度毕业达标线对比" compact />
      </div>
      ...
      <section className="railway-card overflow-hidden"><div className="border-b border-railway-600/50 p-5"><p className="eyebrow">STUDENT DRILLDOWN</p><h2 className="section-heading">学生列表与毕业达标情况</h2><p className="mt-1 text-xs text-text-muted">当前筛选 {visibleStudents.length} 人</p></div><div className="divide-y divide-railway-600/50">{visibleStudents.map((item: any) => <button key={item.id} onClick={() => setStudentId(item.id)} className="grid w-full gap-3 p-4 text-left hover:bg-railway-700/40 sm:grid-cols-[1.3fr_auto_auto_auto_auto] sm:items-center"><div><p className="font-semibold text-text-primary">{item.name}</p><p className="text-xs text-text-muted">{item.student_no}</p></div><span className="text-text-secondary">{item.training_count} 次实训</span><span className="font-mono text-accent-cyan">均分 {item.average_score}</span><span className="font-mono text-text-secondary">达标 {item.graduation_ready_count}/{item.graduation_total_count}（{item.graduation_progress}%）</span><span className={item.graduation_ready ? 'text-status-success' : 'text-status-warning'}>{item.graduation_ready ? '毕业标准已达成' : '毕业风险'} →</span></button>)}{!visibleStudents.length && <p className="p-6 text-center text-sm text-text-muted">当前筛选条件下没有学生</p>}</div></section>
    </>}
...
function StudentDetail({ data, loading, onClose }: { data: any; loading: boolean; onClose: () => void }) {
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-railway-900/90 p-3 backdrop-blur" onClick={onClose}><section className="glass-panel-bright max-h-[92vh] w-full max-w-6xl overflow-y-auto" onClick={(e) => e.stopPropagation()}><header className="sticky top-0 z-10 flex justify-between border-b border-railway-600/50 bg-railway-800/95 p-5"><div><p className="eyebrow">STUDENT COMPREHENSIVE DETAIL</p><h2 className="section-heading">{data?.student?.name || '学生综合详情'}</h2><p className="mt-1 text-xs text-text-muted">{data?.student?.class_name} · {data?.student?.student_no}</p></div><button className="railway-button" onClick={onClose}>关闭</button></header>{loading ? <p className="p-8 text-text-muted">正在汇总学生数据…</p> : data && <div className="space-y-6 p-5"><div className="grid gap-3 sm:grid-cols-5"><Metric label="实训次数" value={data.score_summary.total} /><Metric label="平均成绩" value={data.score_summary.average_score || 0} /><Metric label="综合能力" value={data.ability?.total_score || 0} /><Metric label="毕业达标进度" value={`${data.ability?.graduation_ready_count || 0}/${data.ability?.graduation_total_count || 0}`} tone={data.ability?.graduation_ready ? 'good' : 'warn'} /><Metric label="环境检查" value={data.environment_checks.length} /></div><section className={data.ability?.graduation_ready ? 'rounded border border-status-success/30 bg-status-success/5 p-4' : 'rounded border border-status-warning/30 bg-status-warning/5 p-4'}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">GRADUATION ASSESSMENT</p><h3 className="section-heading">毕业标准评估</h3></div><strong className={data.ability?.graduation_ready ? 'text-status-success' : 'text-status-warning'}>{data.ability?.graduation_ready ? '整体已达标' : '存在毕业风险'}</strong></div><p className="mt-2 text-sm text-text-secondary">已达标 {data.ability?.graduation_ready_count || 0} 项 / 全部 {data.ability?.graduation_total_count || 0} 项，整体进度 {data.ability?.graduation_progress || 0}%</p></section><div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><AbilityRadar items={data.ability?.radar_data || []} title="个人能力画像" description="当前能力值与毕业标准对比" compact /><section className="railway-card p-5"><p className="eyebrow">TRAINING EVIDENCE</p><h3 className="section-heading">近期实训成绩</h3><div className="mt-4 space-y-2">{data.score_summary.scores.slice(0, 8).map((item: any) => <div key={item.id} className="flex justify-between rounded bg-railway-800/60 p-3 text-sm"><span>{item.project_name}</span><span className="text-accent-cyan">{item.percentage} 分</span></div>)}</div></section></div><div className="grid gap-4 md:grid-cols-2"><Metric label="诊断报告" value={data.reports.length} unit="份" /><Metric label="环境检查记录" value={data.environment_checks.length} unit="条" /></div></div>}</section></div>
}
